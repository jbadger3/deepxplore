...
def model(x):
    with tf.variable_scope('lenet5'):
        ...
        c1 = tf.layers.conv2d(x_image,filters=6,kernel_size=[5,5],padding='SAME', activation=tf.nn.relu,name='c1')
        s2 = tf.layers.average_pooling2d(c1, pool_size=[2,2],strides=[2,2],padding='VALID',name='s2')
        c3 = tf.layers.conv2d(s2, filters=16,kernel_size=[5,5],padding='VALID',activation=tf.nn.relu,name='c3')
        s4 = tf.layers.average_pooling2d(c3, pool_size=[2,2],strides = [2,2],padding='VALID',name='s4')
        c5 = tf.layers.conv2d(s4, filters=120,kernel_size=[5,5],padding='VALID',activation=tf.nn.relu,name='c5')
        c5_flat = tf.reshape(c5,[-1,120])
        f6 =tf.layers.dense(c5_flat,84,activation=tf.nn.relu,name='f6')

        #add all activations to collection
        tf.add_to_collection('neurons',c1)
        tf.add_to_collection('neurons',s2)
        tf.add_to_collection('neurons',c3)
        tf.add_to_collection('neurons',s4)
        tf.add_to_collection('neurons',c5)
        tf.add_to_collection('neurons',f6)

        y_logits = tf.layers.dense(f6,10,activation=tf.nn.sigmoid,name='outputs')
    return y_logits
