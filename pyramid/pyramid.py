...
def model(x):
    with tf.variable_scope('pyramid'):
        ...
        regularizer = tf.contrib.layers.l2_regularizer(scale=5.0)
        c1 = tf.layers.dense(batch_norm,196,kernel_regularizer=tf.nn.l2_loss, activation=tf.nn.relu,name='pyramid1')
        c2 = tf.layers.dense(c1,98,kernel_regularizer=tf.nn.l2_loss, activation=tf.nn.relu,name='pyramid2')
        c3 = tf.layers.dense(c2,49,kernel_regularizer=tf.nn.l2_loss, activation=tf.nn.relu,name='pyramid3')
        shape = c3.get_shape().as_list()
        dim = np.prod(shape[1:])
        cflat = tf.reshape(c3, [-1,dim])
        #add all activations to collection
        tf.add_to_collection('neurons',c1)
        tf.add_to_collection('neurons',c2)
        tf.add_to_collection('neurons',c3)


        y_logits = tf.layers.dense(cflat,10,activation=tf.nn.softmax,name='outputs')
    return y_logits
