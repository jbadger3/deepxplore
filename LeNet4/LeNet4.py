...
def model(x):
    with tf.variable_scope('lenet4'):
        x_image = tf.reshape(x, [-1, 28, 28, 1])
        #place all variables on parameter server to share
        #create all trainable variables for the model
        c1 = tf.layers.conv2d(x_image,filters=4,kernel_size=[5,5],padding='SAME', activation=tf.nn.relu,name='c1')
        ...
        y_logits = tf.layers.dense(c5_flat,10,activation=tf.nn.sigmoid,name='outputs')
    return y_logits
